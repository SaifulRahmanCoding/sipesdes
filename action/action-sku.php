...
if ($opsi == "input") {
	...
	// batas
	if (isset($_POST['bidangUsaha'])) {$bidangUsaha = $_POST['bidangUsaha']; } else{echo "kesalahan dari bidangUsaha"; }
	// batas
	...
	// batas
	if (isset($_POST['keterangan'])) {$keterangan = ucwords($_POST['keterangan']); } else{echo "kesalahan dari keterangan"; }
	...
	if($insert_register==false) { ?>
		<script type='text/javascript'>
			...
			window.location.href="../form-sku.php";
		</script>
	<?php } 

	$query = "INSERT INTO sku(no_surat, pekerjaan, keterangan, keperluan, jenis_sku) VALUES ('$no_surat','$pekerjaan','$keterangan','$keperluan','$bidangUsaha')";
	...
	$query = "UPDATE sku SET pekerjaan = '$pekerjaan',keterangan='$keterangan', keperluan = '$keperluan', jenis_sku='$bidangUsaha' WHERE no_surat = '$no_surat'";
	$update_sku = mysqli_query($db,$query);
