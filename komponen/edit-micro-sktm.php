<?php
// query panggil data
$query = "SELECT * FROM tb_register_surat INNER JOIN sktm ON tb_register_surat.no_surat = sktm.no_surat WHERE tb_register_surat.no_surat='$no_surat'";
...
foreach ($result as $sktm) {
	...
	$nik = $sktm['nik'];
	$nikah=$sktm['nikah'];
	$id_pejabat=$sktm['id_pejabat'];
	$pekerjaan = $sktm['pekerjaan'];
	$keperluan = $sktm['keperluan'];
}
?>
<div class="wrap shadow-sm p-3 mt-3">
	<form action="action/action-sktm.php?opsi=update" method="POST">
		<div class="row ps-3">
			<!-- kiri -->
